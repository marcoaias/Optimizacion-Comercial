...
\begin{align}
g(\theta) = \dfrac{1}{2} - \int_{\mu_i}^{\theta_i} \underset{(\overline{\mu}_i, \, \overline{\sigma}_i)}{f(x)}\dif x - \dfrac{\text{tarifa}_{i + 1}}{\text{tmp}_i}  = 0 \label{eq:gtheta}
\end{align}
...
\begin{align}
\delta(\theta) = \sum_{k=1}^\theta \text{Tarifa}_{\left\{\min(i) \, \mid \, k \, < \, \text{Protección}_i^*\right \}}- \left(\left\lceil\dfrac{\theta}{n_{\text{plazas}}}\right\rceil \cdot \frac{\text{Coste}}{\text{vagón}} + \theta\cdot\frac{\text{Tasa}}{\text{pasajero}} \right)
\end{align}
...
\addplot [red, ultra thick] table{vprob0.dat};
\addplot [ForestGreen, densely dashdotted, thick] table{vprob1.dat};
...
\caption{3er y 4o vagón}\label{fig:distvag}
\end{figure}
