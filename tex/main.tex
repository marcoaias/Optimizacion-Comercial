...
Como tal nos ocupa entonces calcular esta probabilidad gaussiana.

La distribución gaussiana \eqref{eq:fgauss} está normalizada (si $\mu = 0$ y $\sigma = 1$) tal que la integral en \nolinebreak[4] $(-\infty, +\infty)$ iguala a \nolinebreak[4] $1$:
...
La integral \eqref{eq:intgauss} no se puede resolver de forma analítica, sino por métodos computacionales de integración numérica, series de Taylor, etc. En concreto la aproximaremos por el \emph{método del trapecio} y también por el método de Simpson.\\
