...
\begin{align}
P(t) = \dfrac{1}{2} + \int_\mu^t f(x)\dif x \label{eq:probgauss}
\end{align}
...
\addplot [green, thick] table {data1.dat};
\addlegendentry{$(0,1)$}
\addplot [yellow, thick] table {data2.dat};
...
\begin{align}
\dfrac{\text{tarifa}_{i + 1}}{\text{tmp}_i} =  P(x > \theta_i)
\end{align}
...
legend style={nodes={scale=0.5, transform shape}},
xlabel={$t$},
ylabel={$\underset{(\mu,\, \sigma)}{1 - P(t)}$},
ymax=1.1,
%ymin=5,
xmax=3,
xmin=-3,
...
\begin{align}
g(\theta) = \int_{\mu_i}^{\theta_i} \underset{(\mu_i, \, \sigma_i)}{f(x)}\dif x + \dfrac{\text{tarifa}_{i + 1}}{\text{tmp}_i} - \dfrac{1}{2} = 0 \label{eq:gtheta}
\end{align}
...
legend style={nodes={scale=0.5, transform shape}},
xlabel={$\theta$},
ylabel={$\underset{(\mu_i,\, \sigma_i)}{1 - P(\theta)}$},
ymax=1.1,
%ymin=5,
xmax=3,
xmin=-3,
